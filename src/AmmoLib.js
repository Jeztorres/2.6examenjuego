...
// Ammo se carga globalmente desde el script en el HTML
const _Ammo = () => Promise.resolve(window.Ammo);
...
class AmmoHelper{
  ...
  static CastRay(world, origin, dest, result={}, collisionFilterMask=CollisionFilterGroups.AllFilter){
    // Crear nuevo raycast cada vez para compatibilidad con CDN de Ammo.js
    const from = new Ammo.btVector3(origin.x, origin.y, origin.z);
    const to = new Ammo.btVector3(dest.x, dest.y, dest.z);
    const rayCallback = new Ammo.ClosestRayResultCallback(from, to);

    // Configurar máscara de colisión si está disponible
